...
router.get('/sleep/:userId', async (req, res) => {
  ...
  try {
    ...
    res.status(200).send(user.sleep);
  } catch (error) {
...
// POST a new sleep record to a user
router.post('/sleep/:userId', async (req, res) => {
  const { userId } = req.params;
  ...
  try {
    ...
    user.sleep.push(sleepRecord);
    await user.save();

    res.json({ message: 'Sleep record saved successfully', user });
  } catch (error) {
    res.status(500).json({ message: 'Error saving sleep record', error });
  }
});

// PATCH (update) a sleep record for a user
router.patch('/sleep/:userId/:sleepId', async (req, res) => {
  try {
    ...
    // find the db sleep record with an id that matches the input sleep record
    for (let i = 0; i < user.sleep.length; i++) {
      if (user.sleep[i]._id === sleepId) {
        // replace the old values with the updated ones
        for (let key in sleepRecord) {
          user.sleep[i][key] = sleepRecord[key];
        }
    ...
    await user.save();
    res.status(200).json({ message: 'Sleep record updated successfully', user });
  } catch (error) {
    console.error(error);
...
router.delete('/sleep/:userId/:sleepId', async (req, res) => {
  try {
    const { userId, sleepId } = req.params;
    ...
    user.sleep = user.sleep.filter(
      (sleepRecord) => sleepRecord._id.toString() !== sleepId
    );

    await user.save();
    res.status(200).json({ message: 'Sleep record deleted successfully', user });
  } catch (error) {
    console.error(error);
